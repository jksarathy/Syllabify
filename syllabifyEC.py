...
for line in file.readlines():
    ...
    while i < len(words)-1:
        ...
        #check if the current phenome is a consonant in the onset of the syllable
             ...
             # EXTRA CREDIT: if S is preceded by a vowel or K and followed by a consonant, it joins coda
            if i > 0 and after in son and current == "S" and (son[after] == 4 or after == "K") and son[words[i-1]] !=4:
                words.insert(i, "+")
                plus = i 
                i +=1

            # EXTRA CREDIT: separate ending syllable of LIY (not including BLIY or TLIY)
            elif after in son and i == 1 and current == "L" and words[i-1] == "IY" and after != "B" and after != "T":
                words.insert(i + 1, "+")
                plus = i + 1
                i +=1
                
            #check if the following syllable isnt S and is a vowel or a valid consonant according to the rule of onsets
            elif after in son and after != "S" and (son[after] == 4 or (son[after] < 4 and son[current] - son[after] < 2)):
                words.insert(i + 1, "+")
                plus = i+1
                i +=1
